...
void Mesh::set_vertex_attribute(const char *attrib_name, GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) {
	if(!shader) {
		return;
	}
	glBindVertexArray(vao);
	...
	glVertexAttribPointer(loc, size, type, GL_FALSE, stride, (void*)pointer);
...
void Mesh::draw(GLenum mode, unsigned int index_buffer) {
	...
	glDrawElements(mode, num_indices[index_buffer], GL_UNSIGNED_SHORT, (void*)0);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
}
